...
{% else %}
1. Download a custom recovery - you can download [Lineage Recovery](https://updater.oddsolutions.us/{{ custom_recovery_codename }}). Simply download the latest recovery file, named something like `lineage-{{ device.current_branch }}-{{ site.time | date: "%Y%m%d" }}-recovery-{{ custom_recovery_codename }}.img`.
{% endif %}
{% endif %}
